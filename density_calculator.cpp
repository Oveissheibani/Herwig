...
// ============================================================================
// PAIR ANALYZER – deterministic tensor product
// ============================================================================
class PairAnalyzer {
private:
    int pdg1, pdg2;
...
public:
            ...
            for (int by1 = 1; by1 <= rho1->GetNbinsY(); ++by1) {
                ...
                for (int bx2 = 1; bx2 <= rho2->GetNbinsX(); ++bx2) {
                    ...
                    for (int by2 = 1; by2 <= rho2->GetNbinsY(); ++by2) {
                        ...
                        // Skip same bin for identical particles → removes artificial delta-peak
                        if (is_self && bx1 == bx2 && by1 == by2) continue;

                        double dy = yc1 - yc2;
                        double dphi = phic1 - phic2;
                        while (dphi >= TMath::Pi()) dphi -= 2 * TMath::Pi();
                        while (dphi < -TMath::Pi()) dphi += 2 * TMath::Pi();

                        // weight = density1 × density2 × bin_area
    ...
    void computeC2(int class_id) {
        ...
        // tensor is already the correct per-event density (integrated properly)
    ...
    void finalize(HistogramManager* hm) {
        for (int c = 0; c < n_classes; ++c) {
            ...
            if (h_c2_cumulant[c]) {
                ...
                TH2F* copy = (TH2F*) h_c2_cumulant[c]->Clone(name.c_str());
                hm->writeC2(c, copy);
                std::cout << "[C2 written] " << name
                          << "  mean = " << copy->GetMean(1)
                          << "  entries = " << copy->GetEntries() << std::endl;
                delete copy;
            }
            ...
            // tensor normalized (already close to correct units)
    ...
    void computeDerivedPhysics() {
        ...
        for (auto& kv : pair_map) {
            ...
            for (int c = 0; c < n_classes; ++c) {
                ...
                if (!s_assoc) continue;
                double N_beta = s_assoc->getMeanMultiplicity(c);
                ...
                std::cout << "[A2 written] " << a2_name
                          << " N_beta = " << N_beta
                          << " mean(C2) = " << c2->GetMean(1)
                          << " mean(A2) = " << h_a2->GetMean(1) << std::endl;
                delete h_a2;
            }
        }
    }
